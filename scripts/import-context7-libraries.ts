...
TOP_LIBRARIES.forEach((lib, i) => {
  ...
  console.log(`INSERT INTO libraries (id, name, description, categories, source_type, source_url, index_status, is_featured, created_at, updated_at)`);
  console.log(`VALUES ('${id}', '${escapeSQL(lib.name)}', '${escapeSQL(lib.query)}', '${JSON.stringify(categories)}', 'context7', '${sourceUrl}', 'pending', ${i < 30 ? 1 : 0}, datetime('now'), datetime('now'))`);
