...
import type { Message, Todo } from "@/lib/opencode/types";
...
function MessageBubble({ 
  ...
  const fileParts = message.parts.filter(
    (p) => 
      ...
      ((p as any).name?.includes("file") || (p as any).name?.includes("edit") || (p as any).name?.includes("read"))
  );
  ...
  fileParts.forEach((p) => {
    if (p.type === "tool-call") {
      ...
      if (toolCall.args) {
        const path = (toolCall.args as any).path || (toolCall.args as any).filePath;
