...
export const statsCommand = new Command("stats")
  ...
  .action(async (options: StatsOptions) => {
    ...
    try {
      ...
      logger.log(chalk.bold.cyan("  │  ") + chalk.bold(`Nexus Stats for ${auth.email}`.padEnd(50)) + chalk.bold.cyan("│"));
      logger.log(chalk.bold.cyan("  ╰──────────────────────────────────────────────────────╯"));
      logger.newline();

      // Plan info
      const planColor = stats.plan === "pro" ? chalk.green : stats.plan === "team" ? chalk.blue : chalk.gray;
      logger.log(`  Plan: ${planColor(stats.plan?.toUpperCase() || "FREE")}${stats.subscription?.status ? ` (${stats.subscription.status})` : ""}`);
      ...
      logger.log(`  │ Queries this month: ${String(usageUsed).padStart(6)} / ${String(usageLimit).padEnd(10)} │`);
      ...
      logger.log(chalk.bold("  ├─────────────────────────────────────────────────────┤"));
      logger.log(`  │ API Keys:          ${String(stats.apiKeys?.count || 0).padStart(3)} / ${String(stats.apiKeys?.limit || 1).padEnd(14)} │`);
      logger.log(`  │ Installed Skills:  ${String(stats.skills?.installed || 0).padStart(3)}`.padEnd(54) + "│");
      logger.log(`  │ Memories:          ${String(stats.memories?.count || 0).padStart(3)} / ${String(stats.memories?.limit || "∞").padEnd(14)} │`);
      logger.log(`  │ Secrets:           ${String(stats.secrets?.count || 0).padStart(3)}`.padEnd(54) + "│");
      logger.log(chalk.bold("  └─────────────────────────────────────────────────────┘"));
      logger.newline();
      ...
      if (stats.plan === "free" && usagePercent >= 80) {
        ...
        logger.info(`Upgrade to Pro for unlimited queries: ${chalk.cyan("https://nexus.yogan.dev/pricing")}`);
        logger.newline();
      }
...
function createProgressBar(percent: number, width: number): string {
  ...
  const filledChar = percent >= 90 ? chalk.red("█") : percent >= 70 ? chalk.yellow("█") : chalk.green("█");
